...
string ::
  Chars
  -> Parser Chars
string xs = reverse <$> traverse is (reverse xs)
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok fst snd = between (charTok fst) (charTok snd)
...
hex ::
  Parser Char
hex = replicateA 4 (satisfy isHexDigit) >>= \x -> 
                                                P $ \i ->
                                                let res = readHex $ reverse x
                                                in case res of
                                                    Full c -> Result i (chr c)
                                                    _ -> UnexpectedEof
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 elemP sepP = do
            first <- elemP
            rest <- list (sepP >>= const elemP)
            return $ first :. rest
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby elemP sepP = (sepby1 elemP sepP) ||| (P $ \i -> Result i Nil)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll ps = satisfyMany ps and
            

satisfyMany :: List (Char -> Bool) -> (List Bool -> Bool) -> Parser Char
satisfyMany ps listOp = P $ \i ->
                    case i of
                        (c :. cs) ->
                            let result = listOp $ sequence ps c
                            in if result
                                then Result cs c
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ps = satisfyMany ps or
