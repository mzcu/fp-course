...
instance Functor ListZipper where
  (<$>) g (ListZipper l x r) = ListZipper (g <$> l) (g x) (g <$> r)
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l x r) =  let (lo, hi) = break p l
                                     left = reverse $ take (length left - 1) (reverse hi)
                                 in case hi of
                                      Nil -> IsNotZ
                                      (u :. us) -> IsZ $ ListZipper us u (reverse lo ++ (x :. Nil) ++ r)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l x r) =  let (lo, hi) = break p r
                                      right = reverse $ take (length hi - 1) (reverse hi)
                                  in case hi of
                                      Nil -> IsNotZ
                                      (u :. us) -> IsZ $ ListZipper (reverse lo ++ (x :. Nil) ++ l) u us
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (u :. us) x r) = ListZipper us u (x :. r)
moveLeftLoop (ListZipper Nil x r) = let focus = take 1 (reverse r)
                                        left  = (reverse $ take (length r - 1) r) ++ (x :. Nil)
                                    in case focus of 
                                        (f :. _) -> ListZipper left f Nil
                                        _ -> error "there must be a focus"
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l x (u :. us)) = ListZipper (x :. l) u us
moveRightLoop (ListZipper l x Nil) = let focus = take 1 (reverse l)
                                         right  = (reverse $ take (length l - 1) l) ++ (x :. Nil)
                                    in case focus of 
                                        (f :. _) -> ListZipper Nil f right
                                        _ -> error "there must be a focus"
...
swapLeft (ListZipper (u :. us) x r) = IsZ $ ListZipper (x :. us) u r
...
swapRight (ListZipper l x (u :. us)) = IsZ $ ListZipper l u (x :. us) 
...
moveLeftN'' m n z
        | n == 0 = Right z
        | n > 0 = case next of
                    IsZ next' -> moveLeftN'' (m+1) (n-1) next'
                    _ -> Left m
        | otherwise = moveRightN' (-n) z
        where
            next = moveLeft z
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n z = moveRightN'' 0 n z
...
moveRightN'' m n z
        | n == 0 = Right z
        | n > 0 = case next of
                    IsZ next' -> moveRightN'' (m+1) (n-1) next'
                    _ -> Left m
        | otherwise = moveLeftN' (-n) z
        where
            next = moveRight z
...
nth n z@(ListZipper l _ _)
                    | pos == n = IsZ z
                    | pos < n = moveRightN (n - pos) z
                    | pos > n = moveLeftN (pos - n) z
                    where pos = length l
...
end (ListZipper l x (u :. us)) = end $ ListZipper (x :. l) u us
...
start (ListZipper (u :. us) x r) = start $ ListZipper us u (x :. r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper l1 x1 r1) (ListZipper l2 x2 r2) = ListZipper (zipWith ($) l1 l2) (x1 x2) (zipWith ($) r1 r2)
...
instance Extend ListZipper where
  (<<=) f z@(ListZipper l x r) = ListZipper (extend hasLeft moveLeft) (f z) (extend hasRight moveRight)
                                    where
                                        extend p movement = unfoldr (\a -> 
                                                    if p a 
                                                        then (\new -> (f new, new)) <$> (toOptional $ movement a)
                                                        else Empty
                                                      ) z
