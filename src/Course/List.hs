...
-- TODO: this implementation is too ugly

seqOptional Nil = Full Nil
seqOptional (Empty :. _) = Empty
seqOptional xs = if length (filter (\x -> case x of Empty -> True; _ -> False) xs) > 0 
                then Empty
                else Full $ map (\x -> case x of Full x -> x; _ -> error "boom") xs
