...
instance Applicative ExactlyOne where
  ...
  (<*>) :: 
    ...
    -> ExactlyOne a
    -> ExactlyOne b
  (<*>) (ExactlyOne f) (ExactlyOne x) = pure $ f x
-- | Insert into a List.
--
-- prop> \x -> pure x == x :. Nil
--
...
lift4 ::
  ...
  -> f d
  -> f e
lift4 f u v w x = lift3 f u v w <*> x
...
--
filtering ::
  Applicative f => Show a =>
  (a -> f Bool)
  -> List a
  -> f (List a)
filtering _ Nil = pure Nil
filtering f (x :. xs) = (\p qs -> if p then x:.qs else qs) <$> f x <*> filtering f xs
